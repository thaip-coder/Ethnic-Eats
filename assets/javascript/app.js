...
auth.onAuthStateChanged(firebaseUser => {
  if (firebaseUser) {
    ...
    // populate selected favorites to the DOM when logged in
    if (firebaseUser != null) {
      ...
      database.ref('/users/' + uid).on("child_added", function (childSnapshot) {
        ...
        var matAdd = $("<a class='btn-floating halfway-fab waves-effect waves-light red'><i id='delete' data-key='" + key + "'class='material-icons delete'>delete</i></a>");
...
function ajaxCall() {
  ...
  $.ajax({
    ...
    for (var i = 0; i < 4; i++) {
      ...
      //building cards to display recipes
      ...
      //add search results to array to be called when selecting a favorite
      var faves = new addFavorites(foodImage, foodDescription, foodURL, counter);
      //push selected favorites to the array
      favoritesArray.push(faves);
      //console.log(response);

      //publish cards to the DOM
...
//Constructor function to build fav recipe objects
...
// on click function to add a favorite
$(document.body).on("click", ".add-favorite", function () {
  ...
  //writes selected favorite to firebase with uid tag
  database.ref('/users/' + uid).push(newRecipe);
});
